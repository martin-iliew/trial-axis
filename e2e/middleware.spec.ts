...
test.describe("Route protection (unauthenticated)", () => {
  test("redirects /cro/projects to /login", async ({ page }) => {
  ...
  test("redirects /cro/projects/new to /login", async ({ page }) => {
  ...
  test("allows access to landing page", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("link", { name: "TrialAxis" })).toBeVisible();
